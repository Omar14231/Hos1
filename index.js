...
const commands = [
    new SlashCommandBuilder().setName('اضافه').setDescription('إضافة رتبة').addStringOption(o => o.setName('ملصق').setDescription('ايموجي أو ملصق').setRequired(true)).addRoleOption(o => o.setName('رتبه').setDescription('اختر الرتبة').setRequired(true)),
    new SlashCommandBuilder().setName('حذف').setDescription('حذف رتبة').addRoleOption(o => o.setName('رتبه').setDescription('اختر الرتبة').setRequired(true))
].map(c => c.toJSON());
    ...
    if (message.content === '!اعداد' && message.author.id === OWNER_ID) {
        ...
        keys.forEach((roleId, index) => {
            currentRow.addComponents(
                new ButtonBuilder().setCustomId(`role_${roleId}`).setLabel(rolesData[roleId].name).setEmoji(rolesData[roleId].emoji).setStyle(ButtonStyle.Primary)
            );
...
client.on('interactionCreate', async interaction => {
    if (interaction.isCommand()) {
        ...
        if (interaction.commandName === 'اضافه') {
            ...
            interaction.reply({ content: `✅ تم إضافة: ${role.name} بملصق ${emoji}`, ephemeral: true });
        } else if (interaction.commandName === 'حذف') {
            const role = interaction.options.getRole('رتبه');
            delete rolesData[role.id];
            interaction.reply({ content: `🗑️ تم حذف الرتبة من القائمة.`, ephemeral: true });
        }
    }
    ...
    if (interaction.isButton() && (interaction.customId.startsWith('yes_') || interaction.customId.startsWith('no_'))) {
        ...
        if (action === 'yes') {
            try {
                ...
                interaction.update({ content: '❌ خطأ: تأكد أن رتبة البوت أعلى من الرتبة المطلوبة.', components: [] });
            }
        } else {
            interaction.update({ content: '❌ تم الإلغاء.', components: [] });
        }
    }
});
